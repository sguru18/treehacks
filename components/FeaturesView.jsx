...
import FeatureDesigner from "@/components/FeatureDesigner";
...
export default function FeaturesView() {
  ...
  const removeFeature = useProductStore((s) => s.removeFeature);
  ...
  return (
    <motion.div variants={stagger} initial="hidden" animate="show" className="max-w-3xl mx-auto">
      ...
      <motion.div variants={fadeUp} className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 mb-0.5">Features</h1>
          <p className="text-sm text-gray-400">
            {features.length > 0
              ? `${features.length} feature${features.length !== 1 ? 's' : ''} designed from ${insights.length} insights`
              : 'Design features one at a time with Daisy'}
          </p>
        </div>
        <div className="flex items-center gap-2">
          {features.length > 0 && onRoadmap.size < features.length && (
            <button
          ...
          {onRoadmap.size > 0 && (
            ...
            >
              Roadmap ({onRoadmap.size})
            </button>
          )}
        </div>
      </motion.div>

      {/* Feature Designer — always visible */}
        ...
        {features.map((feature, index) => {
          ...
          return (
            <motion.div
              key={feature.id}
              variants={fadeUp}
              onClick={() => !isEditing && selectFeature(feature.id)}
              className={`w-full card-hover rounded-xl p-5 text-left group ${isEditing ? '' : 'cursor-pointer'}`}
            >
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[11px] text-gray-400 font-medium tabular-nums">#{index + 1}</span>
                    <span className={`px-1.5 py-0.5 rounded text-[10px] font-semibold ${cat.bg} ${cat.color}`}>
                      {cat.label}
                    </span>
                    {feature.tasks && (
                    ...
                    <div className="ml-auto flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      {isEditing && (
                        <button
                          onClick={(e) => { e.stopPropagation(); if (confirm('Remove this feature?')) removeFeature(feature.id); }}
                          className="px-2 py-0.5 rounded text-[10px] font-medium text-red-500 hover:bg-red-50 transition-colors"
                        >
                          Remove
                        </button>
                      )}
                      <button
                        onClick={(e) => { e.stopPropagation(); setEditingId(isEditing ? null : feature.id); }}
                        className={`px-2 py-0.5 rounded text-[10px] font-medium transition-colors ${
                          isEditing ? 'bg-brand-100 text-brand-700' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
                        }`}
                      >
                        {isEditing ? 'Done' : 'Edit'}
                      </button>
                  ...
                  {isEditing ? (
                    <div className="space-y-2 mb-3" onClick={(e) => e.stopPropagation()}>
                      ...
                      >
                        <p className="text-[12px] text-gray-400 leading-relaxed">{feature.description}</p>
                      </EditableField>

                      {/* Editable scores */}
                      <div className="grid grid-cols-3 gap-3 pt-1">
                        {[
                          { key: 'impact', label: 'Impact', color: 'brand' },
                          { key: 'effort', label: 'Effort', color: 'amber' },
                          { key: 'confidence', label: 'Confidence', color: 'blue' },
                        ].map(({ key, label, color }) => (
                          <div key={key}>
                            ...
                            <div className="flex items-center gap-1.5">
                              <input
                                ...
                                max={10}
                                value={feature[key] || 5}
                                onChange={(e) => updateFeature(feature.id, { [key]: parseInt(e.target.value) })}
                                className={`flex-1 h-1 accent-${color}-500`}
                              />
                              <span className={`text-[11px] font-semibold tabular-nums w-4 text-${color}-600`}>{feature[key]}</span>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <>
                      ...
                      <div className="grid grid-cols-3 gap-x-4 gap-y-1">
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] text-gray-400 w-14 shrink-0">Impact</span>
                          <Bar value={feature.impact || 0} color="bg-brand-500" />
                          <span className="text-[10px] text-gray-500 tabular-nums w-3">{feature.impact}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] text-gray-400 w-14 shrink-0">Effort</span>
                          <Bar value={feature.effort || 0} color="bg-amber-400" />
                          <span className="text-[10px] text-gray-500 tabular-nums w-3">{feature.effort}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] text-gray-400 w-14 shrink-0">Confidence</span>
                          <Bar value={feature.confidence || 0} color="bg-blue-400" />
                          <span className="text-[10px] text-gray-500 tabular-nums w-3">{feature.confidence}</span>
                        </div>
                      </div>
                    </>
                  )}
                ...
                <div className="flex flex-col items-end gap-2 shrink-0 pt-1">
                  ...
                  {!isEditing && (
                    <svg className="w-4 h-4 text-gray-300 group-hover:text-gray-500 transition-colors" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
      ...
      {features.length === 0 && (
        <div className="card rounded-xl p-10 text-center mt-2">
          <p className="text-gray-400 text-[12px]">No features yet. Use the designer above to create your first one.</p>
        </div>
      )}
    </motion.div>
  );
