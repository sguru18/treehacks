...
const INTEGRATIONS = [
  { id: "salesforce", name: "Salesforce", color: "#00A1E0", connected: true, records: "12.4k", lastSync: "2 min ago", status: "healthy" },
  { id: "zendesk", name: "Zendesk", color: "#03363D", connected: true, records: "8.2k", lastSync: "5 min ago", status: "healthy" },
  { id: "intercom", name: "Intercom", color: "#286EFA", connected: true, records: "3.1k", lastSync: "12 min ago", status: "healthy" },
  { id: "slack", name: "Slack", color: "#E01E5A", connected: false, records: null, lastSync: null, status: "disconnected" },
  { id: "gong", name: "Gong", color: "#7C3AED", connected: false, records: null, lastSync: null, status: "disconnected" },
  { id: "hubspot", name: "HubSpot", color: "#FF7A59", connected: false, records: null, lastSync: null, status: "disconnected" },
];

const TEAM_ACTIVITY = [
  { user: TEAM_MEMBERS[1], action: "added 3 new insights from Zendesk analysis", time: "10 min ago", icon: "insight" },
  { user: TEAM_MEMBERS[2], action: "moved Task Dependencies to Sprint 1", time: "25 min ago", icon: "roadmap" },
  { user: TEAM_MEMBERS[3], action: "uploaded new wireframes for Workload Dashboard", time: "1 hr ago", icon: "design" },
  { user: TEAM_MEMBERS[4], action: "ran sentiment analysis on 2.4k tickets", time: "2 hr ago", icon: "data" },
  { user: TEAM_MEMBERS[0], action: "generated spec for Calendar Integration", time: "3 hr ago", icon: "spec" },
];

const INSIGHT_CATEGORIES = [
  { label: "Feature Requests", count: 42, pct: 38, color: "bg-brand-500" },
  { label: "Pain Points", count: 31, pct: 28, color: "bg-amber-500" },
  { label: "UX Issues", count: 19, pct: 17, color: "bg-rose-500" },
  { label: "Performance", count: 12, pct: 11, color: "bg-blue-500" },
  { label: "Integrations", count: 7, pct: 6, color: "bg-violet-500" },
];

const PRIORITY_BREAKDOWN = [
  { label: "Critical", count: 3, color: "bg-red-500" },
  { label: "High", count: 7, color: "bg-orange-500" },
  { label: "Medium", count: 12, color: "bg-amber-400" },
  { label: "Low", count: 5, color: "bg-gray-300" },
];
...
/* ------------------------------------------------------------------ */
/*  TaskFlow logo (the demo company's product)                          */
/* ------------------------------------------------------------------ */
...
export default function Dashboard() {
  ...
  const totalRecords = hasData ? "23.7k" : "0";
  const connectedCount = hasData ? 3 : 0;
  const insightCount = insights.length || (hasData ? 0 : 0);
  const featureCount = features.length || 0;
  const sprintVelocity = roadmapItems.length > 0 ? Math.round(roadmapItems.reduce((acc, r) => acc + (r.storyPoints || 0), 0) / Math.max(1, new Set(roadmapItems.map(r => r.sprint)).size)) : 0;
  const tasksGenerated = features.reduce((acc, f) => acc + (f.tasks?.length || 0), 0);
  ...
  return (
    ...
    >
        ...
        {[
          {
            ...
            value: totalRecords,
            change: hasData ? "+1.2k this week" : "No data yet",
          ...
          },
          {
            ...
            change: hasData ? "3 of 6 active" : "0 connected",
            trend: hasData ? "up" : "neutral",
            icon: (
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor">
          ...
          },
          {
            label: "Insights",
            value: insightCount,
            change: hasInsights ? `${insights.length} extracted` : hasData ? "Ready to extract" : "--",
          ...
          },
          {
            label: "Features",
            value: featureCount,
            change: hasFeatures ? `${tasksGenerated} tasks` : hasInsights ? "Ready to build" : "--",
          ...
          },
          {
            ...
            change: sprintVelocity > 0 ? "pts / sprint avg" : "No sprints yet",
      ...
      {/* ---- Two-column layout: Integrations + Analytics ---- */}
            ...
            {INTEGRATIONS.map((int) => (
              <div
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors ${
                  int.connected ? "bg-gray-50 hover:bg-gray-100" : "opacity-50"
                }`}
              >
                <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-white border border-gray-100 shrink-0">
                  <IntegrationLogo id={int.id} size={20} />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] font-medium text-gray-800">{int.name}</p>
                  <p className="text-[11px] text-gray-400">
                    {int.connected ? `${int.records} records` : "Not connected"}
                  </p>
                </div>
                {int.connected ? (
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] text-gray-400">{int.lastSync}</span>
        ...
        <motion.div variants={fadeUp} className="col-span-2 card rounded-xl p-5">
          ...
          {hasInsights ? (
              ...
              {INSIGHT_CATEGORIES.map((cat) => (
                <div key={cat.label}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-[11px] font-medium text-gray-500 tabular-nums">{insights.length > 0 ? Math.round(insights.length * cat.pct / 100) : cat.count}</span>
                  </div>
            ...
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <p className="text-[11px] text-gray-300">Connect integrations & extract insights</p>
            </div>
          )}
              ...
              {hasData && !hasInsights && (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-amber-600">AI analyze all connected sources</p>
                  </div>
                </button>
              )}
              {hasInsights && !hasFeatures && (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-emerald-600">Prioritize features from insights</p>
                  </div>
                </button>
              )}
              {hasFeatures && (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-violet-600">{roadmapItems.length} items planned</p>
                  </div>
                </button>
              )}
            </div>
          ...
          <div className="card rounded-xl p-5">
            ...
            <div className="space-y-3">
              {[
                ...
                { label: "Plan Sprints", done: features.some((f) => f.tasks), active: hasFeatures },
              ].map((step, i) => (
